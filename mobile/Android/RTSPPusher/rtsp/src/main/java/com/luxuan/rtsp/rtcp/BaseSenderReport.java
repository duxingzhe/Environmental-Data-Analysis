...
public abstract class BaseSenderReport {
    ...
    public abstract void sendReport(byte[] buffer, RtpFrame rtpFrame, String type, int packetCount, int octetCount) throws IOException;
    ...
    private void updateAudio(RtpFrame rtpFrame){
        ...
        if(System.currentTimeMillis()-audioTime>=interval){
            ...
            try{
                sendReport(audioBuffer, rtpFrame, "Video", audioPacketCount, audioOctetCount);
            }catch(IOException e){
                e.printStackTrace();
            }
        }
